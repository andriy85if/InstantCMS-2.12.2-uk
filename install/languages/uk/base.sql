...
(23, 'Wysiwyg редактори', 'wysiwygs', 1, NULL, 'InstantCMS Team', 'https://instantcms.ru', '2.0', 1);
...
(9, 'Видаляє прострочені непідтверджені підписки гостей', 'subscriptions', 'delete_expired_unconfirmed', 1440, 1, DATE_FORMAT(NOW(), '%Y-%m-%d 00:00:05'), 1, 1),
(10, 'Видаляє застарілі сесії', 'users', 'sessionclean', 10, NULL, NULL, 1, 1),
(11, 'Розсилає сповіщення про закінчення публікації', 'content', 'publication_notify', 1440, 1, DATE_FORMAT(NOW(), '%Y-%m-%d 00:00:05'), 1, 1);
...
CREATE TABLE `{#}uploaded_files` (
  ...
  `type` varchar(32) DEFAULT 'file' COMMENT  'Тип файлу',
...
CREATE TABLE `{#}wysiwygs_presets` (
  ...
  `options` text COMMENT 'Опції',
  `title` varchar(100) DEFAULT NULL COMMENT 'Назва пресету',
  PRIMARY KEY (`id`)
